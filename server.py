...
ENV_VARS = [
    ...
    ("OPENROUTER_BASE_URL",       "OpenRouter base URL",      "provider",  False),
    ("OPENAI_API_KEY",           "OpenAI / compatible",      "provider",  True),
    ("OPENAI_BASE_URL",           "OpenAI-compatible base URL", "provider", False),
    ("ANTHROPIC_API_KEY",        "Anthropic",                "provider",  True),
    ("GOOGLE_API_KEY",           "Google AI Studio",         "provider",  True),
    ("GEMINI_API_KEY",           "Gemini (alias/embeddings)", "provider", True),
    ("DEEPSEEK_API_KEY",         "DeepSeek",                 "provider",  True),
    ("DEEPSEEK_BASE_URL",        "DeepSeek base URL",        "provider",  False),
    ("DASHSCOPE_API_KEY",        "DashScope",                "provider",  True),
    ("DASHSCOPE_BASE_URL",       "DashScope base URL",       "provider",  False),
    ("GLM_API_KEY",              "GLM / Z.AI",               "provider",  True),
    ("GLM_BASE_URL",             "GLM base URL",             "provider",  False),
    ("KIMI_API_KEY",             "Kimi",                     "provider",  True),
    ("KIMI_BASE_URL",            "Kimi base URL",            "provider",  False),
    ("MINIMAX_API_KEY",          "MiniMax",                  "provider",  True),
    ("MINIMAX_BASE_URL",         "MiniMax base URL",         "provider",  False),
    ("HF_TOKEN",                 "Hugging Face",             "provider",  True),
    ("HF_BASE_URL",              "Hugging Face base URL",    "provider",  False),
    ("NOVITA_API_KEY",           "NovitaAI",                 "provider",  True),
    ("NOVITA_BASE_URL",          "NovitaAI base URL",        "provider",  False),
    ("NVIDIA_API_KEY",           "NVIDIA NIM",               "provider",  True),
    ("NVIDIA_BASE_URL",          "NVIDIA base URL",          "provider",  False),
    ("XAI_API_KEY",              "xAI",                      "provider",  True),
    ("XAI_BASE_URL",             "xAI base URL",             "provider",  False),
    ("ARCEEAI_API_KEY",          "Arcee AI",                 "provider",  True),
    ("ARCEE_BASE_URL",           "Arcee base URL",           "provider",  False),
    ("GMI_API_KEY",              "GMI Cloud",                "provider",  True),
    ("GMI_BASE_URL",             "GMI base URL",             "provider",  False),
    ("KILOCODE_API_KEY",         "Kilo Code",                "provider",  True),
    ("KILOCODE_BASE_URL",        "Kilo Code base URL",       "provider",  False),
    ("OPENCODE_ZEN_API_KEY",     "OpenCode Zen",             "provider",  True),
    ("OPENCODE_ZEN_BASE_URL",    "OpenCode Zen base URL",    "provider",  False),
    ("OPENCODE_GO_API_KEY",      "OpenCode Go",              "provider",  True),
    ("OPENCODE_GO_BASE_URL",     "OpenCode Go base URL",     "provider",  False),
    ("AWS_REGION",               "AWS region (Bedrock)",     "provider",  False),
    ("AWS_PROFILE",              "AWS profile (Bedrock)",    "provider",  False),
    ("BEDROCK_BASE_URL",         "Bedrock base URL",         "provider",  False),
    ...
    ("TAVILY_BASE_URL",          "Tavily base URL",          "tool",      False),
    ("SEARXNG_URL",              "SearXNG URL",              "tool",      False),
    ("EXA_API_KEY",              "Exa (search)",             "tool",      True),
    ...
    ("BROWSER_USE_API_KEY",      "Browser Use",              "tool",      True),
    ("GITHUB_TOKEN",             "GitHub token",             "tool",      True),
    ("VOICE_TOOLS_OPENAI_KEY",   "OpenAI (voice/TTS)",       "tool",      True),
    ("GROQ_API_KEY",             "Groq (STT)",               "tool",      True),
    ("ELEVENLABS_API_KEY",       "ElevenLabs (TTS)",         "tool",      True),
    ("MISTRAL_API_KEY",          "Mistral (voice)",          "tool",      True),
    ("HONCHO_API_KEY",           "Honcho (memory)",          "tool",      True),
    ("SUPERMEMORY_API_KEY",      "Supermemory",              "tool",      True),
    ...
    ("DISCORD_ALLOWED_ROLES",    "Allowed Role IDs",         "discord",   False),
    ("DISCORD_ALLOWED_CHANNELS", "Allowed Channel IDs",      "discord",   False),
    ("SLACK_BOT_TOKEN",          "Bot Token (xoxb-...)",     "slack",     True),
    ("SLACK_APP_TOKEN",          "App Token (xapp-...)",     "slack",     True),
    ("SLACK_ALLOWED_USERS",      "Allowed User IDs",         "slack",     False),
    ("WHATSAPP_ENABLED",         "Enable WhatsApp",          "whatsapp",  False),
    ("WHATSAPP_ALLOWED_USERS",   "Allowed phone numbers",    "whatsapp",  False),
    ...
    ("EMAIL_ALLOWED_USERS",      "Allowed email addresses",   "email",     False),
    ...
    ("MATRIX_ALLOWED_USERS",     "Allowed Matrix users",     "matrix",    False),
    ("SIGNAL_HTTP_URL",          "Signal HTTP URL",          "signal",    False),
    ("SIGNAL_ACCOUNT",           "Signal account",           "signal",    False),
    ("SIGNAL_ALLOWED_USERS",     "Signal allowed users",     "signal",    False),
    ("TWILIO_ACCOUNT_SID",       "Twilio Account SID",       "sms",       True),
    ("TWILIO_AUTH_TOKEN",        "Twilio Auth Token",        "sms",       True),
    ("TWILIO_PHONE_NUMBER",      "Twilio phone number",      "sms",       False),
    ("SMS_WEBHOOK_URL",          "SMS webhook URL",          "sms",       False),
    ("SMS_ALLOWED_USERS",        "SMS allowed users",        "sms",       False),
    ("WEBHOOK_ENABLED",          "Enable webhooks",          "webhook",   False),
    ("WEBHOOK_PORT",             "Webhook port",             "webhook",   False),
    ("WEBHOOK_SECRET",           "Webhook secret",           "webhook",   True),
    ("API_SERVER_ENABLED",       "Enable API server",        "api_server",False),
    ("API_SERVER_HOST",          "API server host",          "api_server",False),
    ("API_SERVER_PORT",          "API server port",          "api_server",False),
    ("API_SERVER_KEY",           "API server bearer key",    "api_server",True),
    ("API_SERVER_CORS_ORIGINS",  "API server CORS origins",  "api_server",False),
    ("API_SERVER_MODEL_NAME",    "API server model name",    "api_server",False),
    ("GATEWAY_PROXY_URL",        "Gateway proxy URL",        "gateway",   False),
    ("GATEWAY_PROXY_KEY",        "Gateway proxy key",        "gateway",   True),
    ("MESSAGING_CWD",            "Messaging working directory", "gateway", False),
...
def write_env(path: Path, data: dict[str, str]) -> None:
    ...
    cat_labels = {
        ...
        "sms": "SMS", "webhook": "Webhooks", "api_server": "API Server",
        "gateway": "Gateway",
    }
